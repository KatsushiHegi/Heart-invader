...
public class FileDesignation : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        fileAttribute._extensionFilter = (FileDesignationAttribute.FILEEXTENSION)EditorGUI.EnumPopup(rectFileExtension,fileAttribute._extensionFilter);
        if(GUI.Button(rectFileOpenButton, "..."))
        {
            ...
            if(fileAttribute._extensionFilter.ToString() == "NONE")
            {
                pathName = EditorUtility.OpenFilePanel("Select ",directoryPath, "");
            }
            else
            {
                pathName = EditorUtility.OpenFilePanel("Select ", directoryPath  ,fileAttribute._extensionFilter.ToString());
            }
